...
void server::Mode(Client &client, const Command &command)
{
    ...
    if (mode != "+i" && mode != "-i" &&
        ...
        mode != "+o" && mode != "-o")
    {
        sendNumeric(client, ERR_UNKNOWNMODE, client.getNickname() + " " + mode, "Unknown MODE flag");
        return ;
    }
    ...
    else if (mode == "+k")
    {
        ...
        {
            sendNumeric(client, ERR_NEEDMOREPARAMS, client.getNickname() + " MODE", "Not enough parameters");
            return ;
        }
    ...
    else if (mode == "+o" || mode == "-o")
    {
        ...
        {
            sendNumeric(client, ERR_NOTONCHANNEL, client.getNickname() + " " + nickname, "Target is not on that channel");
            return ;
        }
        if (mode == "+o")
    ...
    std::string reply = ":" + client.getNickname() + "!" + client.getUsername() + "@localhost MODE " + channelName + " " + mode;
    if (command.params.size() >= 3)
